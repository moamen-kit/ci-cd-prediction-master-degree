...
The most directly comparable recent work is that of Sharma, Petrova and Connolly [27], who train XGBoost, Random Forest and Support Vector Machine classifiers on 116,000 workflows drawn from 25,000 open-source projects and report a failure-class F1 score of 0.88 at an accuracy of 89.7 per cent. That figure is far above the one this project reports, and the reason is instructive rather than incidental. The authors identify their most influential predictor, by SHAP attribution, as build duration, followed by the number of commits, test coverage volatility and repository age. Build duration is not available until the build has finished. A predictor that depends on it can characterise a failure after the compute has been spent but cannot act to avoid it, which is precisely the boundary this project declines to cross. Their result should therefore be read as an upper bound on what is achievable when post-execution telemetry is permitted, not as a target that the pre-execution setting has failed to reach. The same paper is also a useful illustration of the metric concern raised in Section 7.3: an accuracy of 89.7 per cent is close to the rate a constant prediction of success attains on a corpus with this project's class balance.
